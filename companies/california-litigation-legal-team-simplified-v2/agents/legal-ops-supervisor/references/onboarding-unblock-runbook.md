...
- If direct control-plane status update is unavailable, post a final comment that starts with `Disposition: done` or `Disposition: blocked`, explains the evidence, and names any product/tooling owner.
...
- If the only remaining problem is issue-status mutation, tell the user that no lawyer action is needed and route the problem to the AttyMate product owner.
