...
- `source-intake-gates.md` — green/yellow vs. hard-gate criteria for source intake, pleading review, and OCR.
...
2. **Lane files specialize, never loosen.** A lane gates file (calendar/docket/email monitoring, source intake) may only narrow the matrix — enumerate what is green inside an approved profile or source scope and add lane-specific hard gates. It may never mark green something the matrix hard-gates.
