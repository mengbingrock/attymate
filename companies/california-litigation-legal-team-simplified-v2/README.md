...
Matter context is reusable but relevance-based. Legal Ops should create or confirm a Matter Home at `{workspace}/Matters/{matter-short-name}/` when a matter/output root is approved, with issue audit output under `_issues/{issue-identifier}/`. Specialists should check only the context artifacts relevant to their role and assignment. See `references/matter-context-artifacts.md` for the folder convention, tiered checking rule, and role defaults.
...
Each agent is defined by a single `AGENTS.md` (role, triggers, handoffs, deliverables, decision rights, escalation, principles, and runtime/tooling notes).
...
From a running control-plane instance:

```powershell
npx.cmd <control-plane-cli> company import https://github.com/mengbingrock/attymate/tree/master/companies/california-litigation-legal-team --target new --new-company-name "California Litigation Legal Team" --include company,goals,agents,projects,tasks,skills --yes --api-base http://127.0.0.1:3100
```

For local development from a checked-out repository:

```powershell
npx.cmd <control-plane-cli> company import .\companies\california-litigation-legal-team --target new --new-company-name "California Litigation Legal Team" --include company,goals,agents,projects,tasks,skills --yes --api-base http://127.0.0.1:3100
```
...
Use `references/onboarding-unblock-runbook.md` when onboarding is blocked by workspace selection, runner policy, or control-plane status-update tooling. The active AttyMate WORKSPACE selection is the deployment source of truth; deployment-specific paths belong in the private Firm Operations Guide and must not be committed to this reusable package.

If an onboarding task is substantively complete but the issue cannot be marked done, record the terminal disposition and route the technical blocker to the AttyMate product owner instead of rerunning the same setup task.
...
- Configure external-tool access before use: approved browser tooling, email provider, calendar provider, Google Drive, Lexis, LASC, external knowledge-base/upload systems, filing, service, and upload/download workflows.
