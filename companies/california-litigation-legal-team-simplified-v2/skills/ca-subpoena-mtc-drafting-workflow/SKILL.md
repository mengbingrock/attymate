...
- A scoped control-plane issue involves California subpoena motion-to-compel drafting or QA.
...
- This control-plane-native wrapper is adapted from reusable workflow rules and contains no client facts, firm templates, private account details, or hardcoded paths.
...
Apply `gating/human-approval-gates.md` in both standard and `sandbox_autopilot` modes: local/source-bound draft artifacts and new output-root working copies are green, and only the three hard gate categories stop execution. `sandbox_autopilot` labels test/demo matters; it is not the only low-friction path.
