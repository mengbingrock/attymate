...
description: Coordinate a low-interruption California subpoena motion-to-compel run while preserving matter selection, Launch Intake, authority discipline, file safety, QA, and human approval gates.
---
...
7. **Map the run in the control plane.** Parent issue: "Run subpoena MTC package for selected matter." Child issues: created dynamically after intake; use `parentId` for each specialist. Use control-plane request-confirmation or approval requests only for hard gates or plan acceptance that truly stops further safe work. Keep both control-plane issue history and matter-local run state current.
...
9. **Use tools within contract.** Use control-plane task and approval tools for coordination. Use local file, OCR, document, browser, legal research, email, calendar, or external connector tools only when the Matter Safety Contract and approval state permit that tool class. Do not rely on hidden memory or unstated local configuration.
10. **Maintain durable status** in the control plane and the approved matter-local run-state artifact after implementation is authorized.
...
- Outputs stay in control-plane issue history or under `{output_root}` after implementation approval.
...
- `gating/human-approval-gates.md`: green/yellow/red checkpoint matrix and review packet format.
