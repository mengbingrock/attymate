...
description: Use when a legal agent must calculate, propose, review, create, update, or verify litigation calendar entries, court deadlines, hearing dates, discovery deadlines, service dates, reminder schedules, or calendar QA. Do not use to write to a calendar, send notices, or apply firm-specific calendaring preferences unless the issue supplies the applicable calendar policy and the required approval has been granted.
---
...
Apply `gating/human-approval-gates.md`: proposed deadline/calendar tables are green, but calendar writes, invites, notifications, and emails remain hard gates.
