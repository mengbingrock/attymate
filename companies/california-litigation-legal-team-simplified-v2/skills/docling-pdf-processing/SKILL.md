...
Apply `gating/human-approval-gates.md` in both standard and `sandbox_autopilot` modes: local PDF/OCR sidecars under `{output_root}` are green, but Azure, Codex vision, or any external upload/remote processing remains a hard gate unless separately approved.
