...
Verify the imported Paperclip routine `run-gmail-monitor` exists, is assigned to `gmail-monitor-agent`, has a schedule trigger, uses `coalesce_if_active` and `skip_missed`, and remains `paused` until the board/operator enables it. If the routine is missing, create it from the company package defaults or mark this setup issue blocked with a product/setup note; do not mark Gmail monitoring configured when no routine exists. The routine output must follow `references/monitoring-report-contract.md` and report findings to Legal Ops Supervisor.
