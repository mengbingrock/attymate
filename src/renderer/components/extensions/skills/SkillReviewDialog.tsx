      ...
      <DialogContent className="max-w-[min(96vw,80rem)] gap-0 overflow-hidden p-0">
        <div className="flex max-h-[85vh] min-h-0 min-w-0 flex-col">
          <DialogHeader className="border-b border-border px-6 py-5">
            <DialogTitle>{t('skillReview.title')}</DialogTitle>
            <DialogDescription>{t('skillReview.description', { reviewLabel })}</DialogDescription>
          </DialogHeader>

          <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-6 py-5">
            {!preview && <p className="text-sm text-text-muted">{t('skillReview.noPreview')}</p>}
                  ...
                  {preview.changes.map((change) => (
                    ...
                    >
                      <div className="mb-3 flex flex-wrap items-center gap-2">
                        ...
                        {change.isBinary && (
                          <Badge variant="destructive">{t('skillReview.binaryBadge')}</Badge>
                        )}
                      </div>
