import type { TeamHttpRuntimeApi } from '../TeamProvisioningApis';
...
describe('TeamProvisioning API binders', () => {
  ...
  it('groups HTTP route controls behind narrow facade ports', async () => {
    ...
    const source = {
      ...
      getTeamAgentRuntimeSnapshot: () => Promise.reject(new Error('not implemented')) as never,
  ...
  it('keeps runtime, runtime-control, task activity, and member lifecycle APIs as separate control surfaces', () => {
    ...
    const runtimeSource: TeamRuntimeApi & TeamHttpRuntimeApi = {
      getRuntimeState: () =>
        Promise.resolve({
          teamName: 'team-bound',
          isAlive: true,
  ...
  it('binds messaging and relay methods to the source object', async () => {
    ...
    const source: MessagingSource = {
      ...
      isCodexLaneRuntimeTeam: () => false,
      isStockClaudeRuntimeTeam: () => false,
      deliverCodexLaneDm: () => Promise.resolve(false),
      deliverStockTeammateDm: () => Promise.resolve(false),
      relayMemberInboxMessages: () => Promise.resolve(0),
