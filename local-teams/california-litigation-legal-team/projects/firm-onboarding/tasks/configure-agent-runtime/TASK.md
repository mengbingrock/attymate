...
name: Configure Paperclip agent runtime
...
Confirm each agent's configuration has model, timeout, grace period, approved auth mechanism, and Legal Ops hiring permission. Verify agent adapter availability, authentication signal, skill discovery, and a hello probe before live matter work. Use Paperclip secret references for sensitive environment values. Add a section-ready update or direct document edit to `firm-operations-guide`.
