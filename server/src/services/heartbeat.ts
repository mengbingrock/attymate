...
export function buildUserChatPromptTemplate(userMessage: string): string {
  ...
  return [
    ...
    "",
    "A user is messaging you through the agent's chat sheet. Reply conversationally on stdout. Your stdout reply IS the response delivered to the user — do NOT post comments on any issue, do NOT change any issue state, and do NOT start new work unless the user explicitly asks.",
    "",
    "Optimize for a fast reply. Latency matters: every API call delays the user. Default to answering immediately from what you already know, and only call the API when the answer genuinely depends on live data you don't have.",
    "",
    "User's message:",
    `${fence}text`,
    userMessage,
    fence,
    "",
    "## Decide first: do you even need a tool call?",
    "",
    "- Greetings, chit-chat, opinions, general questions, or anything you can already answer → just reply. Make ZERO API calls.",
    "- Need live state you don't have (your current queue, a specific issue, your own role/instructions) → make the SINGLE most targeted call below, then answer. Do not chain calls or pre-fetch \"just in case\".",
    "- If one quick call doesn't fully answer it, answer with what you have and say what you'd need to check further, rather than fanning out into more calls.",
    "",
