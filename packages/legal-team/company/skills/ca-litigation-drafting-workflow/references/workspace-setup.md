...
- `{output_root}`: approved intermediary work folder for this task.
- `{output_root}/OCR`: OCR and text sidecars.
- `{output_root}/Drafts`: new draft sections and working copies.
- `{output_root}/Tables`: chronology, task, authority, exhibit, and replacement tables.
- `{output_root}/QA`: checklists, risk notes, and unresolved-input lists.

Rules:

- Keep source roots read-only.
- Keep new artifacts under `{output_root}` unless the task expressly approves another location.
