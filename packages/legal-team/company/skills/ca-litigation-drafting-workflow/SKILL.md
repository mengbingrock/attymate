...
description: Use when a pi drafting or QA agent must draft, revise, organize, or review California litigation work product, including pleadings, motions, memoranda, declarations, meet-and-confer letters, proposed orders, source indexes, task tables, and Word working-copy updates. Do not use to insert unapproved authorities, overwrite user-edited files, finalize, sign, file, serve, or email.
---
...
- Working from the Drafting & Assembly Agent, Legal QA Agent, Facts & Evidence Agent, Source Intake Agent, or a supervisor-delegated drafting delegated task.
- Drafting or revising pleadings, motions, memoranda, declarations, meet-and-confer letters, or proposed orders.
- Building or reviewing source indexes, task tables, QA notes, or Word working-copy updates.
...
Before drafting begins, confirm the task states:
...
If scope, source, or output boundaries are missing, return the missing-field list to the supervisor. If approved sources and output root are clear, continue safe draft work and log unresolved tasks instead of blocking.
...
1. **Checkout the assigned task.** Read the matter record, comments, contract, and approved sources.
2. **Build or update intermediary tables** before major drafting when needed (source indexes, task tables, chronology tables, authority tables).
...
5. **Post draft text, task tables, or artifact paths for review.**
6. **Hit the checkpoint before any hard-gate action** (see gates below). Proceed autonomously on green drafting work — source-bound draft text, outlines, task tables, QA notes, change logs, new artifacts, and new working-copy drafts under `{output_root}`. Route yellow tasks to the Legal Ops Supervisor when facts conflict, child scope needs repair, or a strategy question can be separated from safe drafting. Request approval before:
...
7. **Use tooling within policy.** Use document tooling for new output-root working copies or approved active-draft updates. Use legal research tools only after approval. Use OCR/PDF tools through a scoped PDF/OCR task when extraction quality matters.
...
- Return discrete yellow or hard-gate tasks to the supervisor, but do not stop safe drafting when approved sources and output boundaries support continued work.
- Mark done only after posting the draft artifact and the unresolved-task list.
...
- `references/intermediary-work-product.md`: recommended source indexes, task tables, chronology tables, authority tables, and change logs.
