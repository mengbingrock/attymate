...
description: Use when a pi legal agent must calculate, propose, review, create, update, or verify litigation calendar entries, court deadlines, hearing dates, discovery deadlines, service dates, reminder schedules, or calendar QA. Do not use to write to a calendar, send notices, or apply firm-specific calendaring preferences unless the task supplies the applicable calendar policy and the required approval has been granted.
---
...
- The Calendar Agent or a supervisor-delegated calendaring delegated task must calculate or propose court deadlines, hearing dates, discovery deadlines, service dates, or reminder schedules.
...
- This skill is product-safe: it contains no firm-specific rules, private calendar names, internal URLs, account details, or client facts. Deployment-specific policy must be supplied in the task or a configured `{firm_profile}` reference.

## Inputs

Before work begins, confirm the task states:
...
1. Checkout the assigned task before doing substantive work.
2. Read the task body, comments, matter record, and supplied policy references.
3. Confirm the required task contract and approval state.
...
6. Post a proposed deadline/calendar table to the task or save a new artifact under `{output_root}` if allowed.
...
9. **Apply the checkpoint policy.** Green work proceeds autonomously and is logged: deadline calculations from approved facts, proposed calendar tables, verification notes, and artifact creation under `{output_root}`. Yellow routes to the Legal Ops Supervisor when facts are incomplete, policy conflicts exist, or a deadline depends on draft strategy recommendations. Hard gates require visible approval before the actions listed under Anti-patterns. Return discrete yellow or hard-gate tasks to the supervisor but continue safe proposed-entry work when approved facts allow it.

## Outputs

- Proposed calendar entries, calculation notes, task reports, verification notes, and new artifacts under `{output_root}`. Do not store passwords, PINs, account secrets, or private internal URLs.
- Discrete yellow or hard-gate tasks returned to the Legal Ops Supervisor.
...
- Applying a firm policy that was not supplied in the task or deployment configuration.
