...
- The Source Intake Agent is assigned a scoped intake task for complaints, amended complaints, answers, or proofs of service.
...
Before intake begins, confirm the task states:
...
2. Read task scope, comments, parent context, and approved source roots.
3. Inventory source pleadings and identify extraction/OCR needs. Use PDF/OCR tools when source text is unavailable or unreliable. Treat source PDFs as read-only. Use mailbox/drive connectors only when the deployment and task authorize them.
...
- Discrete yellow or hard-gate tasks returned to the Legal Ops Supervisor for resolution.
...
- Downloading from email/drive systems unless the task authorizes it.
