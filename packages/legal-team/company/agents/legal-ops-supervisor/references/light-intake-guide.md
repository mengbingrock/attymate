...
Use this guide when Legal Ops Supervisor receives a monitor finding, a user-created matter record, or a vague request for legal work.

## Goal

Make intake feel like a short conversation with a capable assistant. The lawyer should answer ordinary questions, not fill out scope forms. Legal Ops translates answers into the internal Matter Safety Contract, Matter Dashboard, Matter Home, approval posture, and delegated tasks.

## Default Path

Start in Light Intake Mode unless the task already contains a complete scope.
...
- Source scope: already-approved monitor summary or task text only.
...
- Output: matter record documents until a Matter Home/output folder is configured.
...
Use `skills/ca-subpoena-mtc-autonomous-runner/references/human-approval-gates.md` as the canonical approval matrix. Proceed on local/source-bound work, output-root artifacts, new output-root working copies, draft recommendations, QA, task updates, and internal routing. Stop only for external side effects; authentication, payment, or legal-authority expansion; and destructive or protected mutation.
