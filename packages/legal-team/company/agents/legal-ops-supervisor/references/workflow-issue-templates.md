...
These are reference templates only. They are not import-time starter tasks. Legal Ops Supervisor should create live delegated tasks dynamically from a user-created matter record and set `parentId` on each child.

## Light Intake Parent Task
...
- Current safe source scope, defaulting to monitor summary or task text only.
- Tentative matter label, if available.
- Desired next step: triage only, open parent intake task, draft a plan, or wait.
...
- Output root.
- Matter Home and `_the pi orchestrator_issues/{task-identifier}` audit-output convention when filesystem output is approved.
...
- Matter Home or test Matter Home path, with task audit output under `_the pi orchestrator_issues/{task-identifier}`.
- Read-only source roots.
...
Each delegated task must include a focused Matter Safety Contract, the role-relevant matter context artifacts from `references/matter-context-artifacts.md`, and the lawyer-facing output standard from `references/lawyer-facing-output-standard.md`. Do not create these children during package import.
