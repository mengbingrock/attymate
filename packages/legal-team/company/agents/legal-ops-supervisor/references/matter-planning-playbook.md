...
1. Check the matter record, Firm Operations Guide matter mapping, and any supplied task context for an existing matter match.
...
3. If the match is reliable, attach the event to the existing matter record and update the Matter Dashboard.
4. If no reliable match exists but the user clearly wants new matter setup, create a new matter record with a tentative label and safe source scope.
5. If neither is clear, ask one plain-language question with practical choices before creating specialist delegated tasks.

When a matter/output root is approved, create or confirm the Matter Home described in `references/matter-context-artifacts.md`. If not approved, keep outputs in matter record documents and mark the Matter Dashboard as `not yet filed into Matter Home`.

## Event Plan

For each matter event, create or update a concise Matter Plan on the matter record. Classify each possible workstream as:

- `create now` - safe, source-bound work can begin with the current scope.
- `schedule/monitor` - timing should be tracked through a proposed deadline table, task monitor, or routine where supported.
...
Use the active Matter Safety Contract, not hidden memory, as the source of authority for delegated tasks. Each delegated task should include the relevant matter context artifact paths from `references/matter-context-artifacts.md`, not the full folder by default.

## Work Packet Before Child Tasks
...
Use the no-child rule from `references/workflow-efficiency-budget.md`: dashboard edits, dedupe, monitor triage, short status answers, and small safe local updates stay on the matter record.
...
| Discovery | Track source-bound discovery tasks; strategy-dependent motions stay conditional. |
...
Do not make the lawyer inspect delegated task chains to know whether a workstream is covered. Link the latest artifact in the dashboard and keep detailed audit notes in delegated tasks.
...
If safe local/source-bound work can continue, create or update the relevant delegated task and continue without asking. If no safe work remains, ask the batched decision and set the parent to `in_review`, not `blocked`, when a real interaction is pending.
...
| Docket change | Docket report, procedural history update, calendar proposal if deadlines may change, Legal Ops triage before substantive delegated tasks |
| Email or calendar monitor finding | Triage summary, matter match/new matter decision, source approval question if needed, then scoped delegated tasks |

## Child Task Rules

- Set `parentId` to the matter matter record.
...
- Place or link lawyer-facing outputs in the Matter Home when approved; keep task audit output under `_the pi orchestrator_issues/{task-identifier}` when filesystem output is used.
...
- Do not create delegated tasks for coordination-only work, dashboard edits, dedupe notes, minor monitor cues, or another plan with no deliverable.
- Do not create every possible downstream drafting task as active work when it depends on lawyer strategy. Create a strategy-confirmation question or a blocked/conditional planning item instead.
...
- verify that every delegated task is done, not needed, linked in Coverage, or blocked by a named pending decision;
