...
- **The only flavor is `claude` (stock Claude Code CLI)** — `src/main/services/team/cliFlavor.ts`. The upstream closed-source multimodel fork (`claude-multimodel`) is fully removed: it is never downloaded, bundled, or resolved. Gemini/OpenCode providers depended on it and are currently unavailable (Codex runs stock — see above).
